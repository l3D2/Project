...
app.use((req, res, next) => {
  ...
  res.on("finish", () => {
    ...
    console.log(
      `${req.ip} | ${req.method} ${req.url} ${res.statusCode} - ${elapsed} ms`
    );
    writeLog(elapsed);
  });
  next();
...
app.post("/api/users", async (req, res) => {
  ...
  try {
    let response = await database.insertUser(
      json.name,
      json.email,
      json.googleId
    );
...
app.post("/api/user", async (req, res) => {
  ...
  console.log(json);
  try {
    let response = await database.getUser(json.email);
...
app.put("/api/user", verifyAPIKey, async (req, res) => {
  let json = await req.json();
  try {
    let response = await database.updateUser(
      json.Google_Id,
      json.Name,
      json.Password,
      json.Line_token,
...
app.post("/api/device", verifyAPIKey, async (req, res) => {
  let json = await req.json();
  try {
    let response = await database.createDevice(
      json.device_name,
      json.apikey,
      json.accID
    );
...
app.put("/api/device", verifyAPIKey, async (req, res) => {
  let json = await req.json();
  try {
    let response = await database.updateDevice(
      json.Owned_ID,
      json.Device_ID,
      json.Device_Name,
      json.Cat_ID,
...
app.get("/api/notify", verifyAPIKey, async (req, res) => {
  ...
  try {
    ...
    if (response.length > 0) res.status(200).json(response);
    else res.status(400).send("Notify not found");
  } catch (err) {
    res.status(500).send(err);
  }
});
...
app.put("/api/notify", verifyAPIKey, async (req, res) => {
  let json = await req.json();
  try {
    let response = await database.updateNoti(
      json.device_id,
      json.status,
      json.dmin,
      json.dmax
...
app.get("/api/report-all", verifyAPIKey, async (req, res) => {
  ...
  try {
    ...
    if (response.length > 0) res.status(200).json(response);
    else res.status(400).send("Report not found");
  } catch (err) {
    res.status(500).send(err);
  }
});
