...
const aggregateDataByDay = (data) => {
  ...
  data.forEach((item) => {
    ...
    aggregatedData[day].ec += parseFloat(item.ec);
    console.log("Day"+{day},aggregatedData[day].ec)
    aggregatedData[day].h += parseFloat(item.h);
    aggregatedData[day].ph += parseFloat(item.ph);
    aggregatedData[day].ta += parseFloat(item.ta);
    aggregatedData[day].t += parseFloat(item.t);
    aggregatedData[day].count += 1;
  });
  console.log("Day",aggregatedData);
...
const aggregateDataByMonth = (data) => {
  ...
  data.forEach((item) => {
    ...
    aggregatedData[month].ec += parseFloat(item.ec);
    aggregatedData[month].h += parseFloat(item.h);
    aggregatedData[month].ph += parseFloat(item.ph);
    aggregatedData[month].ta += parseFloat(item.ta);
    aggregatedData[month].t += parseFloat(item.t);
...
const aggregateDataByYear = (data) => {
  ...
  data.forEach((item) => {
    ...
    aggregatedData[year].ec += parseFloat(item.ec);
    aggregatedData[year].h += parseFloat(item.h);
    aggregatedData[year].ph += parseFloat(item.ph);
    aggregatedData[year].ta += parseFloat(item.ta);
    aggregatedData[year].t += parseFloat(item.t);
...
export default function UserChart({ rdata }) {
  ...
  useEffect(() => {
    const filteredData = filterDataByDate(rdata, startDate, endDate);
  ...
  }, [startDate, endDate, viewMode, rdata]);
